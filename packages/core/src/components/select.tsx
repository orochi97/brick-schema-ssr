...
export const Select = ({ cid, props, styles = {}, value }: SelectProps) => {
  ...
  return <select style={libraries.useStyles(styles)} value={value} onChange={onChange}>
    {
      props?.options?.map(({ value: itemValue, label, disabled }) => {
        const optionStyle = Object.assign({}, disabled ? baseStyle.disabled : {} )
        return <option key={itemValue} value={String(itemValue)} selected={String(itemValue)===value} disabled={disabled} style={libraries.useStyles(optionStyle)}>{label}</option>
      })
    }
  </select>
};
